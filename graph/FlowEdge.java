public class FlowEdge {
	...
	public FlowEdge(int v, int w, double capacity) {
		...
		if (!(capacity >= 0.0)) throw new IllegalArgumentException("Edge capacity must be non-negative integer");
		this.v = v;
		this.w = w;
	...
	public FlowEdge(int v, int w, double capacity, double flow) {
		...
		if (!(flow >= 0)) throw new IllegalArgumentException("flow must be non-negative");
	...
	public void addResidualFlowTo(int vertex, double delta) {
		...
		if (!(flow <= capacity)) throw new IllegalArgumentException("Flow is exceeds capacity");
	}
